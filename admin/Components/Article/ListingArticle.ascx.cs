...
public partial class Admin_Components_Article_ListingArticle : BaseUserControl
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!string.IsNullOrEmpty(BicSession.ToString("ArticleCommentNew")))
            ddlCommentNew.SelectedValue = BicSession.ToString("ArticleCommentNew");
    ...
    {
        MenuUserUtils.BindingRadTreeView(tvMenuUser, ddlLanguage.SelectedValue, "news");
    }
    ...
    {
        var item = e.Item as GridPagerItem;
        if (item == null) return;
        var pager = item;
    ...
    protected void rgManager_ItemCommand(object source, GridCommandEventArgs e)
    {
        try
        {
            switch (e.CommandName)
            {
                ...
                case "IsActive":
                    // Check nếu danh mục chính bị bỏ duyệt thì thông báo
                    if (e.CommandArgument.Equals("False"))
                    {
                        var currentItem = BicConvert.ToInt32(e.Item.OwnerTableView.DataKeyValues[e.Item.ItemIndex]["ArticleID"].ToString());
                        var query =
                            "select MenuUser.IsActive from MenuUser Inner join Article on MenuUser.MenuUserID = Article.MainMenuUserID where Article.ArticleID = " +
                            currentItem + " and MenuUser.MenuUserID = Article.MainMenuUserID";
                        var data = new DataHelper();
                        var dt = data.ExecuteSQL(query);
                        if (dt.Rows.Count == 0)
                            ScriptManager.RegisterStartupScript(Page, typeof(Page), "alet", "alert('" + BicResource.GetValue("Admin", "Admin_Article_Message9") + "')", true);
                        var st = dt.Rows[0]["IsActive"].ToString();
                        if (st.Equals("False"))
                        {
                            ScriptManager.RegisterStartupScript(Page, typeof(Page), "alet", "alert('" + BicResource.GetValue("Admin", "Admin_Article_Message8") + "')", true);
                            break;
                        }
                    }
    ...
    protected void btnFind_OnClick(object sender, EventArgs e)
    {
        ...
        BicSession.SetValue("ArticleCommentNew", ddlCommentNew.SelectedValue);
        ...
        else
        {
            foreach (GridDataItem item in rgManager.SelectedItems)
            {
                if (item.Selected)
                {
                    ...
                    if (articleEntity.IsActive && Approved == false)
                        BicAjax.Alert(string.Format(BicResource.GetValue("Admin", "Admin_Article_Message1")));
                    else
                    {                    
                        ArticleBiz.DeleteArticle(id);
