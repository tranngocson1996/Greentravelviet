<%@ Control Language="C#" AutoEventWireup="true" CodeFile="ImageManager.ascx.cs" Inherits="admin_Components_ImageGallery_ImageManager" %>
<%@ Import Namespace="BIC.Utils" %>
<telerik:RadCodeBlock runat="server">
    <%= IncludeAdmin.DefaultCss() %>
    <%= IncludeAdmin.JqueryUI() %>
    <%= IncludeAdmin.AdminImageManager() %>
    <%= IncludeAdmin.JsImageGallery() %>
...
<telerik:RadAjaxPanel ID="RadAjaxPanel1" runat="server" LoadingPanelID="RadAjaxLoadingPanel1">
    ...
    <div class="body_view">
        ...
        <div class="galery_note">
            <%-- ( Hướng dẫn: Click đúp lên ảnh để chèn ảnh )--%>
            <telerik:RadCodeBlock ID="RadCodeBlock1" runat="server">
                ( <%= BicResource.GetValue("Admin", "Admin_Gallery_Guid") %> )
            </telerik:RadCodeBlock>
        </div>
        <div class="text_result">
            <asp:Label runat="server" ID="lblMessage" Text="Lbc"></asp:Label>
