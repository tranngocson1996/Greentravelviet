...
function EditImage(name, imgid) {
    var lang = $("#ImageManager1_hdfLangManager").val();
            ...
            if (confirm("Bạn chắc chắn muốn xóa các ảnh đã chọn?")) $.ajax({
                ...
                success: function (mess) {
                    if (parseInt(mess) == 0) {
                        ...
                        if (location.href.match("ismulti")) { window.location = getBaseURL() + "admin/Components/ImageGallery/Gallery.aspx?ismulti=0&l=" + lang; }
                        else {
                            window.location = getBaseURL() + "admin/Components/ImageGallery/Gallery.aspx?l=" + lang;
                        }
                    } else {
                        alert("Có " + mess + " ảnh xóa không thành công.");
                        if (location.href.match("ismulti")) {
                            window.location = getBaseURL() + "admin/Components/ImageGallery/Gallery.aspx?ismulti=0&l=" + lang;
                        } else { window.location = getBaseURL() + "admin/Components/ImageGallery/Gallery.aspx?l=" + lang; }
                    }
                },
