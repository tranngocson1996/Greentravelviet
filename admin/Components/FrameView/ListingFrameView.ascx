<%@ Control Language="C#" AutoEventWireup="true" CodeFile="ListingFrameView.ascx.cs" Inherits="Admin_Components_FrameView_ListingFrameView" %>
        ...
        <div class="grid">
            ...
            <telerik:RadCodeBlock ID="RadCodeBlock1" runat="server">
                <script type="text/javascript">
                    function RowGridSelected(sender, args) { key = args.getDataKeyValue("FrameViewID"); document.getElementById("radGridSelectedRowIndex").value = key; }
            ...
            <telerik:RadGrid ID="rgManager" EnableEmbeddedSkins="false" Skin="BICCMS" runat="server" HeaderStyle-CssClass="rgHeader rgHeaderCenter" AllowPaging="True"
                ...
                <MasterTableView TableLayout="Fixed" NoMasterRecordsText="Kh&ocirc;ng c&oacute; d&#7919; li&#7879;u..." DataKeyNames="FrameViewID" CommandItemDisplay="Top">
                    <CommandItemTemplate>
                        ...
                        <a href='<%# BicAdmin.UrlAdd() %>' class="btn-addnew" visible="<%# Added %>"><%=BicResource.GetValue("Admin","Admin_FrameView_AddNew") %></a>
                    </CommandItemTemplate>
                    <Columns>
                        ...
                        <telerik:GridTemplateColumn HeaderText="<%$Resources:Admin, Admin_FrameView_FunctionName%>">
                            <ItemTemplate>
                                <%#BicResource.GetValue("Admin",BicConvert.ToString(Eval("Name")))%>
                            </ItemTemplate>
                        </telerik:GridTemplateColumn>
                        <telerik:GridBoundColumn HeaderText="<%$Resources:Admin, Admin_FrameView_Law%>" DataField="URLControl" />
                        <telerik:GridBoundColumn HeaderText="<%$Resources:Admin, Admin_FrameView_StyleFunction%>" DataField="TypeOfControl" HeaderStyle-Width="10%" ItemStyle-HorizontalAlign="Center" />
                        <telerik:GridBoundColumn HeaderText="<%$Resources:Admin, Admin_FrameView_ResourceKey%>" DataField="ResourceKey" />
                        <telerik:GridBoundColumn HeaderText="Đường dẫn (trang danh sách)" DataField="NewColumn1" />
                        <telerik:GridBoundColumn HeaderText="Đường dẫn (trang chi tiết)" DataField="NewColumn2" />
                        <telerik:GridTemplateColumn HeaderText="<%$Resources:Admin, Admin_FrameView_Browse%>" UniqueName="TemplateColumn" HeaderStyle-Width="50px">
                            <ItemTemplate>
