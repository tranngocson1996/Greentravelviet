...
public partial class Admin_Components_Transportation_ListingTransportation : BaseUserControl
{
...
if (txtSearch.Text != String.Empty)
			bicData.Conditioning.Add(new ConditioningItem(TransportationEntity.FIELD_TENPHUONGTIEN,"%"+BicConvert.ToString(txtSearch.Text)+"%", Operator.LIKE, CompareType.STRING));
if (ddlIsActive.SelectedValue != "2")
    ...
    {
        var id = BicConvert.ToInt32(e.Item.OwnerTableView.DataKeyValues[e.Item.ItemIndex]["TransportationID"]);
        TransportationBiz.DeleteTransportation(id);
        GetDataSource();
        rgManager.DataBind();
    }
...
if (ddlCurrentPosition != null){TransportationBiz.PositionWithPriorityEdit(ddlCurrentPosition);ddlCurrentPosition.SelectedValue = BicConvert.ToString(drv["Priority"]);}
...
if (ibtnIsActive != null)ibtnIsActive.Enabled = Approved;
    ...
    protected void radMenuContext_ItemClick(object sender, RadMenuEventArgs e)
    {
        ...
        var id = Convert.ToInt32(rgManager.Items[index].GetDataKeyValue("TransportationID"));
        var transportationEntity = TransportationBiz.GetTransportationByID(id);
        switch (e.Item.Value)
        {
            case "Add":
            ...
            case "View":
                ...
                break;
            case "Delete":
                if (Deleted && transportationEntity.IsActive && Approved == false)
                ...
                break;
            case "Edit":
                  if (Edited && transportationEntity.IsActive && Approved == false)
        ...
        switch (e.CommandName)
        {
			case "Change":
				...
				dhChange.ChangePosition(
					BicConvert.ToInt32(e.Item.OwnerTableView.DataKeyValues[e.Item.ItemIndex]["TransportationID"]), "TransportationID",
				...
				break;
		case "IsActive":
			if (BicRadGrid.ExecuteBooleanCommand(e, e.CommandName, "TransportationId", e.Item.OwnerTableView.DataKeyValues[e.Item.ItemIndex]["TransportationID"].ToString(), "Transportation"))
				GetDataSource();
			else
				 e.Canceled = true;
			rgManager.DataBind();
