...
public partial class admin_Components_Article_RelatedArticle : UserControl
{
    public string Lang
    {
        get
        {
            var obj = ViewState["LanguageKey"];
    ...
    public string RelatedArticleId
    {
        get
        {
            var obj = ViewState["RelatedArticleId"];
            return string.IsNullOrEmpty(Convert.ToString(obj)) ? string.Empty : (string)obj;
        }
    ...
    private DataTable LoadArticle()
    {
        ...
        bicData.Conditioning.Add(new ConditioningItem { Query = "ArticleId in (" + RelatedArticleId + ")", TypeOfCondition = TypeOfCondition.QUERY });
        bicData.Sorting.Add(new SortingItem("(CharIndex(',' + CONVERT(NVARCHAR,ArticleId) +',','," + RelatedArticleId + ",'))", false));
    ...
    public void GetValue()
    {
        ...
        {
            lvRelatedArticle.QueryCondition = string.Format("LanguageKey = '{0}' and TypeOfControl={1} and Title like N'%{2}%'", Lang, 3, txtFilter.Text.Trim());
        }
        else
        {
            lvRelatedArticle.QueryCondition = string.Format("LanguageKey = '{0}' and TypeOfControl={1} ", Lang, 3);
        }
    ...
    protected void chkRelated_OnCheckedChanged(object sender, EventArgs e)
    {
        ...
        RelatedArticleId = relatedArticleIds.ToArray().Aggregate(string.Empty, (current, articleId) => current + ((current == string.Empty ? "" : ",") + articleId));
        lvArticle.DataSource = RelatedList;
        lvArticle.DataBind();
