...
public partial class Admin_Components_Article_ViewArticle : BaseUserControl
{
    ...
    protected string GetNameMenu(string menus)
    {
        ...
        result = arrmenu.Where(s => !string.IsNullOrWhiteSpace(s)).Aggregate(result, (current, s) => current + (MenuUserBiz.GetNameById(BicConvert.ToInt32(s)) + ", "));
        if (result.Length > 2)
    ...
    protected string GetNameCategory(string categories)
    {
        ...
        result = arr.Where(s => !string.IsNullOrWhiteSpace(s)).Aggregate(result, (current, s) => current + (BicXML.ToString(s, "ModelNews_vi") + ", "));
        if (result.Length > 2)
            result = result.Remove(result.Length - 2, 2);
        return result;
    }
