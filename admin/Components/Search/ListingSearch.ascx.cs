...
public partial class Admin_Components_Search_ListingSearch : BaseUserControl
{
    ...
    {
        var bicData = new BicGetData {TableName = "Search"};
        bicData.Sorting.Add(new SortingItem("Priority", false));
        bicData.Selecting.Add(SearchEntity.FIELD_SEARCHID);
        bicData.Selecting.Add(SearchEntity.FIELD_DESCRIPTION);
        bicData.Selecting.Add(SearchEntity.FIELD_KEYWORD);
        bicData.Selecting.Add(SearchEntity.FIELD_IMAGEID);
        bicData.Selecting.Add(SearchEntity.FIELD_LINK);
        bicData.Selecting.Add(SearchEntity.FIELD_PRIORITY);
        bicData.Selecting.Add(SearchEntity.FIELD_ISACTIVE + ",DienThoai");
        if (txtSearch.Text != String.Empty)
            bicData.Conditioning.Add(new ConditioningItem(SearchEntity.FIELD_KEYWORD, "%" + BicConvert.ToString(txtSearch.Text) + "%", Operator.LIKE, CompareType.STRING));
        ...
        {
            bicData.Conditioning.Add(new ConditioningItem("LanguageKey", BicConvert.ToString(ddlLanguage.SelectedValue), Operator.EQUAL, CompareType.STRING));
        }
    ...
    {
        int id = BicConvert.ToInt32(e.Item.OwnerTableView.DataKeyValues[e.Item.ItemIndex]["SearchID"]);
        SearchBiz.DeleteSearch(id);
        GetDataSource();
        rgManager.DataBind();
    }
    ...
    protected void rgManager_ItemDataBound(object sender, GridItemEventArgs e)
    {
        ...
        if (drv == null) return;
        if (imgImageId != null) BicImage.ViewImageFix(imgImageId, BicConvert.ToInt32(drv["ImageID"]), 50, 36, true);
        ...
        {
            SearchBiz.PositionWithPriorityEdit(ddlCurrentPosition);
    ...
    protected void radMenuContext_ItemClick(object sender, RadMenuEventArgs e)
    {
        ...
        int id = Convert.ToInt32(rgManager.Items[index].GetDataKeyValue("SearchID"));
        ...
        switch (e.Item.Value)
        {
            ...
            case "View":
                ...
                break;
            case "Delete":
                if (Deleted && searchEntity.IsActive && Approved == false)
                ...
                break;
            case "Edit":
                if (Edited && searchEntity.IsActive && Approved == false)
    ...
    protected void rgManager_ItemCommand(object source, GridCommandEventArgs e)
    {
        switch (e.CommandName)
        {
            case "Change":
                ...
                dhChange.ChangePosition(BicConvert.ToInt32(e.Item.OwnerTableView.DataKeyValues[e.Item.ItemIndex]["SearchID"]), "SearchID", BicConvert.ToInt32(((DropDownList) e.Item.FindControl("ddlCurrentPosition")).SelectedItem.Text), "Search");
                GetDataSource();
                rgManager.DataBind();
                break;
            case "IsActive":
                if (BicRadGrid.ExecuteBooleanCommand(e, e.CommandName, "SearchId", e.Item.OwnerTableView.DataKeyValues[e.Item.ItemIndex]["SearchID"].ToString(), "Search"))
                    GetDataSource();
                else
                    e.Canceled = true;
                rgManager.DataBind();
