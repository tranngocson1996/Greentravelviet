...
public partial class Admin_Components_GoogleMaps_EditionGoogleMaps : BaseUserControl
{
    protected int Id;
    ...
    protected void LoadXml(int key)
    {
        if (key != 0)
        {
            try
            {
                XDocument xmldoc = XDocument.Load(HttpContext.Current.Server.MapPath("~/admin/XMLData/GoogleMaps.xml"));
                XElement xmlelement = xmldoc.Element("GoogleMaps").Elements("key").Single(x => (int?) x.Attribute("datakey") == key);
    ...
    protected void SaveXml(int key)
    {
        if (key != 0)
        {
            try
            {
                string sRealPath = HttpContext.Current.Server.MapPath("~/admin/XMLData/GoogleMaps.xml");
