...
public partial class Admin_Components_Support_EditionSupport : BaseUserControl
{
    ...
    protected void LoadXML(int key)
    {
        if (key >= 0)
        {
            try
            {
                XDocument xmldoc = XDocument.Load(HttpContext.Current.Server.MapPath(string.Format("~/admin/XMLData/LiveSupport_{0}.xml", BicSession.ToString("Language"))));
                ...
                txtPhone.Text = Server.HtmlDecode(xmlelement.Attribute("phone").Value);
            }
            catch (Exception ex)
            {
    ...
    protected void SaveXML(int key)
    {
        if (key >= 0)
        {
            try
            {
                string mappath = HttpContext.Current.Server.MapPath(string.Format("~/admin/XMLData/LiveSupport_{0}.xml", BicSession.ToString("Language")));
                ...
                xmlelement.Attribute("phone").SetValue(Server.HtmlEncode(txtPhone.Text));
                xmldoc.Save(mappath);
