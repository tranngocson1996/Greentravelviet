<%@ Control Language="C#" AutoEventWireup="true" CodeFile="RelatedProduct.ascx.cs" Inherits="admin_Components_Product_RelatedProduct" %>
<%@ Import Namespace="BIC.Utils" %>
<div class="selected-list">
    <div style="font-weight: bold;">
        <asp:LinkButton ID="LinkButton1" runat="server" OnClick="Unnamed_Click1" Font-Underline="true"><%= BicResource.GetValue("Admin", "Admin_Product_SelectRelatedProductsFromList") %></asp:LinkButton>
    </div>
    <asp:ListView runat="server" ID="lvProduct">
        ...
        <ItemTemplate>
            <li style="list-style: inside disc!important;">
                <%# Eval("Title") %>
                <asp:LinkButton runat="server" ID="productRemove" OnCommand="productRemove_Command" CommandName='<%# Container.DataItemIndex %>' CommandArgument='<%# Eval("ProductId") %>' Title="<%$Resources:Admin,Admin_Product_RemovingNews%>" Font-Underline="true" ForeColor="Red"><img src='<%= Page.ResolveUrl("~/admin/Styles/icon/ContextMenu/delete.gif") %>' /></asp:LinkButton>
            </li>
        </ItemTemplate>
        <EmptyItemTemplate>

            <%= BicResource.GetValue("Admin", "Admin_Product_Noposts") %>
        </EmptyItemTemplate>
        <EmptyDataTemplate>
            <%= BicResource.GetValue("Admin", "Admin_Product_Noposts") %>
        </EmptyDataTemplate>
...
<telerik:RadAjaxPanel runat="server" ID="radAjax">
    <asp:Panel runat="server" ID="listSelect" Style="display: block; height: auto; left: 326px; outline: 0px none; top: 549px; width: 690px; z-index: 1002;" class="ui-dialog ui-widget ui-widget-content ui-corner-all ui-draggable fixed" TabIndex="-1" role="dialog" aria-labelledby="ui-dialog-title-04644c8b64f80a81375df2e2d1" Visible="false">
        <div class="ui-dialog-titlebar ui-widget-header ui-corner-all ui-helper-clearfix">
            <span class="ui-dialog-title" id="Span1"><%= BicResource.GetValue("Admin", "Admin_Product_RelatedProductList") %></span><asp:LinkButton ID="LinkButton2" runat="server" OnClick="Unnamed_Click" class="ui-dialog-titlebar-close ui-corner-all" role="button"><span class="ui-icon ui-icon-closethick"><%= BicResource.GetValue("Admin", "Admin_Product_close") %></span></asp:LinkButton>
        </div>
        <div id="Div1" style="border: 0pt none; bottom: 0pt; display: block; left: 0pt; margin: 0pt; min-height: 0px; overflow: hidden; padding: 0pt; right: 0pt; top: 0pt; width: auto;" class="ui-dialog-content ui-widget-content" scrolltop="0" scrollleft="0">
            <div>
                ...
                <bic:ProductListViewTopPager runat="server" ID="lvRelatedProduct" ShowTodayCss="True" SelectFields="CreatedDate" PageSize="10" OnItemDataBound="lvRelatedProduct_OnItemDataBound">
                    <LayoutTemplate>
                        <div style="display: inline-block; font-family: arial; width: 100%;">
                            ...
                            <div style="border-left: 1px solid #fff; color: #2f5d78; float: left; font-size: 12px; font-weight: bold; height: 36px; line-height: 36px; padding-left: 8px;">
                                <%=BicResource.GetValue("Admin","Admin_Product_NewsTitle") %>
                            </div>--%>
                            </div>
                    ...
                    <ItemTemplate>
                        <div style="background: #DDEBF1; border-left: 1px solid #fff; clear: both; height: 32px; line-height: 32px;">
                            <div style="border-left: 1px solid #fff; float: left; height: 25px; padding-bottom: 1px; padding-top: 5px; text-align: center; width: 25px;">
                                <asp:CheckBox runat="server" Value='<%# Eval("ProductId") %>' Title='<%# Eval("Title") %>' ID="chkRelated" AutoPostBack="True" OnCheckedChanged="chkRelated_OnCheckedChanged" />
                            </div>
                ...
                <div style="background: #9EC4D4; padding: 2px 5px; text-align: left;">
                    <bic:PagerUI runat="server" ID="pRelatedProduct" PagerUIStep="10" Label="Trang: " PageSize="10" OnPageIndexChanged="pRelatedProduct_OnPageIndexChanged" />
                </div>
            </div>
