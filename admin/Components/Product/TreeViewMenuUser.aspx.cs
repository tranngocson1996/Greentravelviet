...
public partial class admin_Components_Product_TreeViewMenuUser : BasePage
{
    protected void Page_Load(object sender, EventArgs e)
    {
        if (IsPostBack) return;
        string lang = BicHtml.GetRequestString("l", "vi");
        MenuUserUtils.BindingRadTreeView(tvMenuUser, lang, "products");
    }
    ...
    protected void MoveItem()
    {
        ...
        try
        {
            string sql = string.Format(@"Update Product set MenuUserId=',{0},' ,MainMenuUserID = {0} ,
            MenuUserName=isnull((select dbo.GetBuildStringToLink(MenuUser.Name,Product.LanguageKey) from MenuUser where MenuUser.MenuUserID = {0}),'')
            where ProductID in ({1})", menuUserID, items);
        ...
        BizObject.PurgeCacheItems("Product_Product");
